...
;
	ldf	a,__idesc__		; data
	ld	yh,a			; start
	ldf	a,__idesc__+1		; address
	ld	yl,a
...
far:
	ldw	c_x,x			; save offset
	ldf	a,(__idesc__+6,x)	; load
	push	a			; end
	ldf	a,(__idesc__+5,x)	; address
	push	a
	popw	x			; convert
	subw	x,#__idesc__		; to offset
	ldw	c_y,x			; in memory
	ldw	x,c_x			; reload offset
	ldf	a,(__idesc__+2,x)	; move ram
	ld	c_x,a			; address in
	ldf	a,(__idesc__+3,x)	; vector
	ld	c_x+1,a
	ldf	a,(__idesc__+4,x)
	ld	c_x+2,a
	pushw	x			; save offset
